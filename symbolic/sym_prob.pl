...
0.45::action(decr_lr,underfitting):- problem(underfitting).
0.4::action(inc_neurons,underfitting):- problem(underfitting).
0.5::action(new_fc_layer, underfitting):- problem(underfitting), \+problem(latency), \+problem(model_size).
0.6::action(new_conv_layer, underfitting):- problem(underfitting), \+problem(latency), \+problem(model_size).
0.55::action(inc_batch_size,floating_loss):- problem(floating_loss).
0.35::action(decr_lr,floating_loss):- problem(floating_loss).
