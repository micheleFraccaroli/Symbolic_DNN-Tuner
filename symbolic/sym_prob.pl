...
0.5::action(data_augmentation,overfitting):- problem(overfitting).
% 0.6::action(decr_lr,underfitting):- problem(underfitting).
...
0.3::action(inc_batch_size,floating_loss):- problem(floating_loss).
